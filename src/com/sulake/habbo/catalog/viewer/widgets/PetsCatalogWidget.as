package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_534 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_534 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2589 = _loc3_.localizationId;
         this.var_371 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_534,this);
               if(_loc5_)
               {
         ...
         this._offers.add(this.var_534,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_709,this.onApproveNameResult);
      ...
      {
         if(param1.productCode == this.var_2589)
         {
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_534 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         while(_loc3_ < this.var_208.length)
         {
            ...
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_534,_loc5_),this.getRaceLocalizationKey(this.var_534,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_208 == null || this.var_811 >= this.var_208.length)
         {
            return;
         }
         var _loc2_:* = 0;
         ...
         var _loc3_:int = this.var_208[this.var_811] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_534,_loc3_,_loc2_,AvatarScaleType.const_55,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_336,AvatarAction.const_906);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         var breed:int = this.var_208[this.var_811] as int;
