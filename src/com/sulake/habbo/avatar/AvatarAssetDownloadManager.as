package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_2104:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_959 = new Timer(this.const_2104,1);
         this.var_959.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_427)
         {
            this.var_427.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_427)
         {
            this.var_427.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         else
         {
            ...
            if(this.var_1476.indexOf("?") > 0)
            {
               _loc2_ = this.var_1476 + "&retry=" + this.var_961;
            }
            else
            {
               _loc2_ = this.var_1476 + "?retry=" + this.var_961;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2484,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1195[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1195[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1846)
         {
            this.var_1194.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_960)
         {
            _loc7_ = true;
            _loc4_ = this.var_960[_loc3_];
      ...
      {
         var _loc1_:* = null;
         while(this.var_785.length > 0 && this.var_784.length < this.const_1786)
         {
            _loc1_ = this.var_785[0];
            _loc1_.startDownloading();
            this.var_784.push(this.var_785.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_785.indexOf(param1) == -1 && this.var_784.indexOf(param1) == -1)
         {
